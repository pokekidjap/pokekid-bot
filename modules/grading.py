...
def _build_status_summary(
    ...
    summary = [
        ...
        f"<b>{html.escape(status)}</b>: <b>{count}</b>"
...
def build_grading_text(
    ...
    if not all_filtered_records:
        if search_query:
            return _grading_response(
                "🎴 <b>RISULTATI RICERCA SUB</b>\n\n"
                f"Nessuna SUB trovata per <code>{html.escape(search_query)}</code>.\n\n"
                "Prova un altro termine o annulla la ricerca."
            )
    ...
    if search_query:
        sections.extend(
            [
                f"🔎 <b>Ricerca:</b> <code>{html.escape(search_query)}</code>",
                "",
            ]
        )
    ...
    sections.extend(
        [
            f"📄 Risultati: <b>{len(all_filtered_records)}</b> · "
    ...
    for section_index, (status, records) in enumerate(grouped_page_records):
        ...
        if section_index < len(grouped_page_records) - 1:
            sections.extend(
                [
                    "━━━━━━━━━━━━━━━━━━",
                    "",
...
def _load_grading_records(
    ...
    if grading_records is None:
        grading_records = get_grading_records(force_refresh=force_refresh)
...
async def _render_grading_page(
    ...
    *,
    ...
    grading_records = _load_grading_records(
        context,
...
async def receive_grading_search(
    ...
    if not search_query:
        await update.message.reply_text(
            "Inserisci un termine valido per la ricerca."
        )
        return GRADING_SEARCH
    ...
    if not request_data:
        await update.message.reply_text(
            "Errore interno durante la ricerca. Riprova."
        )
        return ConversationHandler.END
